...
# specify targets
targets <- c("MMP1", "MMP2", "MMP9","TIMP1","CCL18","THBS1")
...
# specify targets
targets <- c("DMBT1", "SPINK1", "AGR2","OLFM4","REG1A")
...
transposed_matrix <- t(RNA_seurat@assays$RNA@counts)
...
# Move the 'cell_ID' column to the first position
transposed_matrix <- transposed_matrix %>% 
  select(cell_ID, everything())
# Remove the original 'index' column
transposed_matrix <- transposed_matrix[, -ncol(transposed_matrix)]
...
# Reorder the columns as per desired order
merged_matrix <- merged_matrix[, c("fov", "cell_ID", colnames(merged_matrix))]
...
#INDIVIDUAL SUBSETS
```{r}
#Now we will perform pseudobulk analysis with deseq2, the first part is an example of how to analysis an individual cell subset MNP_NOS as an example, and later the pseudobulk analysis across all cells
cts<-AggregateExpression(RNA_seurat,
                    group.by=c("Fine_annotation_3","HS"),
                    assays = 'RNA',
                    slot = 'counts',
                    return.seurat = FALSE)
...
#get values where you want to split
cts.t[1:10,1:10]
splitRows <- gsub('_FOV.*', '', rownames(cts.t))
#split data.trame
cts.split<-split.data.frame(cts.t,
                 f=factor(splitRows))
cts.split$`01-IEC_crypt_base`[1:10,1:10]
#fix colnames and transpose
cts.split.modified<-lapply(cts.split, function(x){
  rownames(x)<-gsub('.*(FOV.*)', '\\1',rownames(x))
                t(x)
})
#gsub('.*(FOV.*)', '\\1','01-IEC_crypt_base_FOV01_HS34_VDZ_LC_POST_2_Mayo0_Responder')
cts.split.modified$`01-IEC_crypt_base`[1:10,1:10]
#DE analysis in 05-MNP_NOS
counts_MNP<-cts.split.modified$`05-MNP_NOS`
colData <- data.frame(samples=colnames(counts_MNP))
colData <- colData %>%
...
colData$condition <- relevel(factor(colData$condition), ref = "PRE_Responder")

#CreateDESeq2 object
dds<-DESeqDataSetFromMatrix(countData=counts_MNP,
                       colData=colData,
                       design = ~ condition)
#filter
...
res <- results(dds, contrast=c("condition", "PRE_Non-responder", "PRE_Responder"))
#write to csv file
...
#ALL CELLS
```{r}
# Aggregate expression by "HS" only
